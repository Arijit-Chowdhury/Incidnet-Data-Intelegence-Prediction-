...
meta = pd.read_csv("D:\ML\inc_1.csv", sep = ',', header = None,encoding='latin-1')
#meta.head()
# rename columns
meta.columns = ["incident_num","description","assignment_group",3]
#meta.head()

inc = pd.DataFrame(meta)

incident=inc[['incident_num','description','assignment_group']]
incident.head()
assigns = [] 

# extract genres
for i in incident['assignment_group']: 
...
multilabel_binarizer.fit(incident['assignment_group'])

# transform target variable
y = multilabel_binarizer.transform(incident['assignment_group'])
#print (multilabel_binarizer)
tfidf_vectorizer = TfidfVectorizer(max_df=0.8, max_features=1000)
# split dataset into training and validation set
xtrain, xval, ytrain, yval = train_test_split(incident['clean_desc'], incident['assignment_group'], test_size=0.05, random_state=1)
